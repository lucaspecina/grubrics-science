...
if [ "$PROFILE" = "workstation" ]; then
    echo "NEXT STEPS (workstation):"
    echo "  1. Generate parquet:"
    echo "     python -m grubrics_science.data.prepare preset --output_dir ./data/processed/ --only-cached"
    echo ""
    echo "  2. Run veRL GRPO (short validation):"
    echo "     python run_grpo.py --config configs/verl_grpo.yaml trainer.total_training_steps=3"
    echo ""
